// --- PROMPT MAESTRO V3.0 - SCRIPT DE LA PLATAFORMA RUTA SABER V12.1 (FINAL CORREGIDO) ---
...
async function initReportPage() {
    ...
    try {
        ...
        const areaKeys = areas.slice(1).map(a => a.toLowerCase());
        ...
        areaKeys.forEach(areaKey => {
            ...
            new ApexCharts(document.querySelector(`#bullet-${areaKey}`), {
                ...
                series: [{ name: 'Rangos', data: nivelesArea.map(n => ({ x: n.nivel, y: n.max - n.min, fillColor: n.color })) }],
                ...
                annotations: {
                    ...
                    xaxis: [
                        { x: nacionalesData.promedios['2024'].calendario_a[areaKey], borderColor: '#775DD0', label: { text: `Nal. A: ${nacionalesData.promedios['2024'].calendario_a[areaKey]}` } },
                        { x: nacionalesData.promedios['2024'].calendario_b[areaKey], borderColor: '#FF4560', label: { text: `Nal. B: ${nacionalesData.promedios['2024'].calendario_b[areaKey]}` } }
                    ]
                },
